...
\xdef\gre@gregoriotexversion{6.2.0-beta3}% GREGORIO_VERSION - VersionManager.py
...
%% Flag and saved dimensions for zeroing staff geometry when lines are hidden.
%% Must be declared before loading gregoriotex-spaces.tex because the
%% calculate macros there check this flag.
\newdimen\gre@dimen@stafflineheight@saved\relax%
\newdimen\gre@dimen@interstafflinespace@saved\relax%
\newdimen\gre@dimen@staffheight@saved\relax%
\newif\ifgre@staffdimensions@zeroed%
\gre@staffdimensions@zeroedfalse%
...
\def\gre@setinitial#1{%
  ...
  \else %
    ...
    \hbox attr \gre@attrid@part=9 {\raise\gre@dimen@annotationtrueraise\box\gre@box@annotation}%
...
% the code is a bit strange here: we always execute \gre@space@dimen@spaceabovelines at the beginning of a glyph. This will:
...
\newcount\gre@abovelinestext@phantomwrapper%
\gre@abovelinestext@phantomwrapper=0\relax%

\def\gresetabovelinestext#1{%
  \gre@processvisibilitywithphantom{#1}%
    {\gre@showabovelinestexttrue}%
    {\gre@showabovelinestextfalse}%
    {\gre@abovelinestext@phantomwrapper}%
    {\protect\gresetabovelinestext}%
}%
...
\def\gre@nabc@prepare#1#2{%
  ...
  \fi%
  \ifgre@nabc@suppress@leftoverflow\else%
    \ifdim\gre@dimen@nabcleftoverflow>\gre@dimen@nabckernemitted\relax%
      \kern\dimexpr(\gre@dimen@nabcleftoverflow-\gre@dimen@nabckernemitted)\relax%
      \global\gre@dimen@nabckernemitted=\gre@dimen@nabcleftoverflow\relax%
    \fi%
...
% Deferred variant of \GreSetNabcAboveLines for voice 1: skips alterations
% and anchors the neume to the first real square note.
\def\GreSetNabcAboveLinesDeferred#1{%
  \global\gre@nabc@voice@i@readyfalse%
  \global\gre@nabc@voice@i@altskippedfalse%
  \gdef\gre@nabc@pre@voice@i{%
    \ifgre@nabc@in@alteration%
      \global\gre@nabc@voice@i@altskippedtrue%
    \else%
      \ifgre@nabc@voice@i@altskipped%
        \gre@nabc@suppress@leftoverflowtrue%
      \fi%
      \gre@nabc@prepare{#1}\gre@box@nabc@voice@i%
...
% Deferred variant of \GreSetNabcBelowLines for voice 2: skips alterations
% and anchors the neume to the first real square note.
\def\GreSetNabcBelowLinesDeferred#1{%
  \global\gre@nabc@voice@ii@readyfalse%
  \global\gre@nabc@voice@ii@altskippedfalse%
  \gdef\gre@nabc@pre@voice@ii{%
    \ifgre@nabc@in@alteration%
      \global\gre@nabc@voice@ii@altskippedtrue%
    \else%
      \ifgre@nabc@voice@ii@altskipped%
        \gre@nabc@suppress@leftoverflowtrue%
      \fi%
      \gre@nabc@prepare{#1}\gre@box@nabc@voice@ii%
      \gre@nabc@suppress@leftoverflowfalse%
...
\gre@showlinestrue
\newcount\gre@lines@phantomwrapper%
\gre@lines@phantomwrapper=0\relax%

%% Helper macros for zeroing/restoring staff dimensions.
%% The @saved values are maintained by the calculate functions, so toggling
%% the flag and recalculating is sufficient; no manual save/restore needed.
\def\gre@staffdimensions@unzero{%
  \ifgre@staffdimensions@zeroed
    \global\gre@staffdimensions@zeroedfalse
...
\def\gre@staffdimensions@dozero{%
  \ifgre@staffdimensions@zeroed\else
    \global\gre@staffdimensions@zeroedtrue
    \gre@calculate@stafflineheight
    \gre@calculate@interstafflinespace
    \gre@calculate@staffheight
    \gre@calculate@constantglyphraise
...
\def\gresetlines#1{%
  \gre@processvisibility{#1}%
    {\gre@showlinestrue}%
    {\gre@showlinesfalse}%
    {\gre@lines@phantomwrapper}%
    {\protect\gresetlines}%
  % When lines are invisible or hphantom, zero the staff dimensions
  % so that all vertical positioning collapses (notes flatten, NABC lowers).
  % When lines are visible or phantom, restore the saved values.
  \ifgre@showlines
    \gre@staffdimensions@unzero
  \else
    \ifnum\gre@lines@phantomwrapper=1\relax
      \gre@staffdimensions@unzero
    \else
      \gre@staffdimensions@dozero
    \fi
  \fi
}
...
\def\gre@generatelines{%
  ...
  \setbox\gre@box@lines=\hbox to 0pt{%
    \vbox attr \gre@attrid@part=2 {% width will be adjusted in Lua
      ...
      \vskip\gre@space@dimen@spaceabovelines\relax%
...
\newcount\gre@translation@phantomwrapper%
\gre@translation@phantomwrapper=0\relax%

\def\gresettranslation#1{%
  \gre@processvisibilitywithphantom{#1}%
    {\gre@showtranslationtrue}%
    {\gre@showtranslationfalse}%
    {\gre@translation@phantomwrapper}%
    {\protect\gresettranslation}%
}%
...
\def\GreWriteTranslation#1{%
  \ifgre@showtranslation%
    \ifgre@translationcentering %
      ...
      \kern\gre@dimen@temp@five %
      \raise\gre@space@dimen@spacebeneathtext\hbox attr \gre@attrid@part=5 to 0pt{\vbox to 0pt{\vss\hbox to 0pt{\gre@applyphantomwrapper{\gre@translation@phantomwrapper}{\gre@style@translation#1\endgre@style@translation}\hss}}}%
      \kern-\gre@dimen@temp@five %
    \else %
      \raise\gre@space@dimen@spacebeneathtext\hbox attr \gre@attrid@part=5 to 0pt{\vbox to 0pt{\vss\hbox to 0pt{\gre@applyphantomwrapper{\gre@translation@phantomwrapper}{\gre@style@translation#1\endgre@style@translation}\hss}}}%
    \fi %
...
\def\GreWriteTranslationWithCenterBeginning#1{%
  \ifgre@showtranslation%
    ...
    \raise\gre@space@dimen@spacebeneathtext\hbox attr \gre@attrid@part=5 to 0pt{\kern 0pt\vbox to 0pt{\vss\hbox to 0pt{\gre@applyphantomwrapper{\gre@translation@phantomwrapper}{\gre@style@translation#1\endgre@style@translation}\hss}}\kern 0pt}%
