...
% Used inside \gre@notes@rendernabc and \gre@notes@rendernabc@withspacing.
...
% Like \gre@noopglyph but preserves horizontal glyph width for NABC
% positioning.  Renders no visible glyph content, but advances the cursor
% by the glyph width and triggers deferred NABC rendering via
% \gre@newglyphcommon.
\def\gre@spacingnoopglyph#1#2#3#4#5#6#7{%
  \gre@newglyphcommon
  \setbox\gre@box@temp@width=\hbox{\gre@font@music #1}%
  \kern\wd\gre@box@temp@width\relax
  \ifgre@firstglyph
    \gre@calculate@notesaligncenter{#4}%
    \global\gre@firstglyphfalse
  \fi
...
% Like \gre@notes@rendernabc but preserves horizontal glyph spacing.
% Used when notes are invisible but NABC is visible, so that NABC neumes
% are correctly positioned horizontally instead of overlapping.
\def\gre@notes@rendernabc@withspacing#1{%
  \begingroup
    \let\GreGlyph\gre@spacingnoopglyph
    \let\GreGlyphHeights\gre@noopglyphheights
    \gre@suppress@bars
    #1%
  \endgroup
  ...
  \else%
    ...
    \else
      ...
      \ifgre@nabc@needsspacing
        \setbox\gre@box@syllablenotes=\hbox{\gre@notes@rendernabc@withspacing{#1}}%
        \gre@compute@nabc@extrakern
      \else
...
\def\gre@syllable@act#1#2#3#4#5#6#7#8#9{%
  ...
  \else%
    ...
    \else
      \gre@check@nabc@needsspacing
      \ifgre@nabc@needsspacing
        % NABC visible: preserve horizontal spacing for proper positioning
        \gre@notes@rendernabc@withspacing{#9}%
        % Apply per-syllable NABC kern (computed in \gre@syllablenotes)
        \ifdim\gre@dimen@nabc@extrakern>0pt\relax
          \kern\gre@dimen@nabc@extrakern
        \fi
