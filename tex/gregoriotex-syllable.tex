...
\gre@declarefileversion{gregoriotex-syllable.tex}{6.2.0-beta1}% GREGORIO_VERSION
...
\def\GreGlyph#1#2#3#4#5#6#7{%
  ...
  \ifgre@notes@invisrender
    \hbox{\phantom{\unhcopy\gre@box@temp@width}}%
  \else
...
\gre@showlyricstrue%
\newcount\gre@lyrics@phantomwrapper%
\gre@lyrics@phantomwrapper=0\relax%

\def\gresetlyrics#1{%
  \gre@processvisibility{#1}%
    {\gre@showlyricstrue}%
    {\gre@showlyricsfalse}%
    {\gre@lyrics@phantomwrapper}%
    {\protect\gresetlyrics}%
}%

\newif\ifgre@shownotes%
\gre@shownotestrue%
\newcount\gre@notes@phantomwrapper%
\gre@notes@phantomwrapper=0\relax%
\newif\ifgre@notes@invisrender%
\gre@notes@invisrenderfalse%

% Helpers to render NABC while suppressing note glyphs
\def\gre@noopglyph#1#2#3#4#5#6#7{\gre@newglyphcommon\gre@endofglyphcommon}%
\def\gre@noopglyphheights#1#2{}%

% Suppress all bar glyph definitions so that bars produce no output.
% Used inside \gre@notes@rendernabc and \gre@notes@rendernabc@tight.
\def\gre@suppress@bars{%
...
\def\gre@notes@rendernabc#1{%
  \begingroup
    \let\GreGlyph\gre@noopglyph
    \let\GreGlyphHeights\gre@noopglyphheights
    \gre@suppress@bars
    #1%
  \endgroup
}%

% Tracks the maximum NABC width for the current syllable across all voices.
% Used to detect when NABC extends past the total glyph width and extra
% horizontal space is needed at the end of the syllable to prevent overlap
% with the next NABC.
\newdimen\gre@dimen@nabc@maxwidth
\gre@dimen@nabc@maxwidth=0pt\relax

% Stores the extra kern needed at the end of the syllable to accommodate
% NABC overflow.  Computed during the pre-measurement pass in
% \gre@syllablenotes and applied after the real notes typesetting in
% \GreSyllable.
\newdimen\gre@dimen@nabc@extrakern
\gre@dimen@nabc@extrakern=0pt\relax

% If NABC extends past the total syllable notes width, compute and save
% the extra kern, then widen \gre@box@syllablenotes accordingly.
% Called after \gre@box@syllablenotes has been filled.  The kern is
% applied at syllable level (not per-glyph) to avoid splitting
% multi-glyph neumes like ce/fgf or f!gwh/ih/hf/fe~.
\def\gre@compute@nabc@extrakern{%
  \ifdim\gre@dimen@nabc@maxwidth>\wd\gre@box@syllablenotes\relax
    \global\gre@dimen@nabc@extrakern=\dimexpr(
      \gre@dimen@nabc@maxwidth - \wd\gre@box@syllablenotes)\relax
    \setbox\gre@box@syllablenotes=\hbox{%
      \unhbox\gre@box@syllablenotes
      \kern\gre@dimen@nabc@extrakern
    }%
  \fi
}%

% Flag to track whether NABC visibility requires horizontal spacing
% preservation when notes are invisible.
\newif\ifgre@nabc@needsspacing

% Check if any NABC voice is visible, meaning horizontal spacing must be
% preserved when notes are invisible to prevent NABC neume overlap.
\def\gre@check@nabc@needsspacing{%
  \gre@nabc@needsspacingfalse
  \ifgre@nabcvoice@i@visible
    \gre@nabc@needsspacingtrue
  \else\ifgre@nabcvoice@ii@visible
    \gre@nabc@needsspacingtrue
  \fi\fi
}%

% Zero-width glyph handler that triggers NABC rendering and computes the
% alignment center for the first glyph, but does not advance the cursor.
% Used by \gre@notes@rendernabc@tight for NABC-only horizontal spacing.
\def\gre@nabconlyglyph#1#2#3#4#5#6#7{%
  \gre@newglyphcommon
  \ifgre@firstglyph
    \gre@calculate@notesaligncenter{#4}%
    \global\gre@firstglyphfalse
...
% Render NABC without preserving any note glyph or inter-element spacing.
% The total horizontal width comes entirely from \gre@compute@nabc@extrakern,
% which expands the syllable box to the NABC width.  This produces tighter
% spacing than the hphantom path because inter-glyph and inter-element glue
% is suppressed.
\def\gre@notes@rendernabc@tight#1{%
  \begingroup
    \let\GreGlyph\gre@nabconlyglyph
    \let\GreGlyphHeights\gre@noopglyphheights
    \gre@suppress@bars
...
\def\gresetnotes#1{%
  \gre@processvisibility{#1}%
    {\gre@shownotestrue}%
    {\gre@shownotesfalse}%
    {\gre@notes@phantomwrapper}%
    {\protect\gresetnotes}%
}%

% Notes-specific phantom wrapper that keeps NABC visible by using
% invisible text rendering for notes and restoring normal rendering inside NABC.
% #1 - phantom wrapper counter (1=phantom, 2=hphantom)
% #2 - content to wrap
\def\gre@applynotesphantom#1#2{%
  \begingroup
    \gre@notes@invisrendertrue%
    \setbox0=\hbox{#2}%
    \ifcase#1\relax
      \box0%
    \or
      \box0%
    \or
      \ht0=0pt\relax \dp0=0pt\relax \box0%
    \else
      \box0%
    \fi
  \endgroup
...
\def\gre@syllablenotes#1{%
  ...
  % Reset NABC tracking for this syllable.
  ...
  \else%
    \ifnum\gre@notes@phantomwrapper>0\relax
      \setbox\gre@box@syllablenotes=\hbox{\gre@applynotesphantom{\gre@notes@phantomwrapper}{#1}}%
    \else
      \gre@check@nabc@needsspacing
      \ifgre@nabc@needsspacing
        \setbox\gre@box@syllablenotes=\hbox{\gre@notes@rendernabc@tight{#1}}%
        \gre@compute@nabc@extrakern
      \else
...
\def\gre@syllable@act#1#2#3#4#5#6#7#8#9{%
  ...
  \else%
    \ifnum\gre@lyrics@phantomwrapper>0\relax
      \setbox\gre@box@syllabletext=\hbox{\gre@applyphantomwrapper{\gre@lyrics@phantomwrapper}{\gre@emit@syllabletext{\gre@pointandclick{\gre@firstsyllablepart\gre@middlesyllablepart\gre@emit@endsyllablepart}{#6}}}}%
    \else
  ...
  \ifgre@showhyphenafterthissyllable\relax%
    ...
    \ifnum\gre@lastoflinecount=1\relax %
      ...
      \else%
        \ifnum\gre@lyrics@phantomwrapper>0\relax
          \setbox\gre@box@syllabletext=\hbox{\gre@applyphantomwrapper{\gre@lyrics@phantomwrapper}{\gre@emit@syllabletext{\gre@pointandclick{\gre@firstsyllablepart\gre@middlesyllablepart\gre@emit@endsyllablepart#3{\GreHyph}\relax}{#6}}}}%
        \else
          \setbox\gre@box@syllabletext=\box\voidb@x%
        \fi
      \fi%
    \else %
      ...
      \else%
        \ifnum\gre@lyrics@phantomwrapper>0\relax
          \setbox\gre@box@syllabletext=\hbox{\gre@applyphantomwrapper{\gre@lyrics@phantomwrapper}{\gre@emit@syllabletext{\gre@pointandclick{\gre@firstsyllablepart\gre@middlesyllablepart\gre@emit@endsyllablepart#3{-}}{#6}}}}%
        \else
          \setbox\gre@box@syllabletext=\box\voidb@x%
        \fi
      \fi%
  ...
  \else%
    \ifnum\gre@notes@phantomwrapper>0\relax
      \gre@applynotesphantom{\gre@notes@phantomwrapper}{#9}%
    \else
      \gre@check@nabc@needsspacing
      \ifgre@nabc@needsspacing
        % NABC visible: render with tight (NABC-only) spacing
        \gre@notes@rendernabc@tight{#9}%
        % Apply per-syllable NABC kern (computed in \gre@syllablenotes)
        \ifdim\gre@dimen@nabc@extrakern>0pt\relax
          \kern\gre@dimen@nabc@extrakern
        \fi
      \else
        % Notes disabled: render NABC without affecting spacing
        \hbox to 0pt{\gre@notes@rendernabc{#9}\hss}%
      \fi
    \fi
  \fi%
...
\def\GreBarSyllable#1#2#3#4#5#6#7#8#9{%
  ...
  \ifgre@newbarspacing%
    ...
    \else%
      \ifnum\gre@notes@phantomwrapper>0\relax
        \gre@applyphantomwrapper{\gre@notes@phantomwrapper}{#9}%
      \else
        \gre@check@nabc@needsspacing
        \ifgre@nabc@needsspacing
          % NABC visible: render with tight (NABC-only) spacing
