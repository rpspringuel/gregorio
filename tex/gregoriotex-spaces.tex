...
\def\gre@calculate@stafflineheight{%
  ...
  \ifgre@staffdimensions@zeroed
    % dimensions intentionally zeroed by \gresetlines, save the new
    % computed value but keep the effective value at 0pt
...
\def\gre@calculate@annotationtrueraise{%
  ...
  %% When staff dimensions are zeroed (lines hphantom/invisible), use the
  %% saved staffheight so the annotation keeps its normal vertical position.
  %% The Lua post_linebreak callback positions the initial independently,
  %% so without this the annotation would drop by staffheight while the
  %% initial stays put, causing overlap.
  \ifgre@staffdimensions@zeroed%
    \global\advance\gre@dimen@annotationtrueraise by %
      \dimexpr(\gre@dimen@stafflineheight@saved %
        * \gre@count@stafflines %
        + \gre@dimen@interstafflinespace@saved %
        * \gre@count@stafflines %
        - \gre@dimen@interstafflinespace@saved %
        + \gre@space@dimen@spacebeneathtext %
