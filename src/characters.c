...
static bool already_visited(const char *lang, char **visited, int count)
{
    int i;

    for (i = 0; i < count; ++i) {
        if (strcasecmp(lang, visited[i]) == 0) {
            return true;
        }
    }
    ...
    do {
        ...
        if (visited_count >= MAX_ALIAS_DEPTH) {
            gregorio_messagef(
                ...
                VERBOSITY_WARNING,
                0,
                _("Alias depth exceeded while resolving %s. "
        ...
        for (p = filenames; (filename = *p); ++p) {
            ...
            char *previous_language = language;

            gregorio_messagef(
                "read_rules",
                VERBOSITY_INFO,
                0,
                _("Looking for %s in %s"),
                language,
                filename
            );
            ...
            case RFPS_FOUND:
                ...
                break;
            case RFPS_ALIASED:
                /* We don't need a message here because one is generated by the lexer */
                
                /* Recheck to see if the alias points to latin
                 * that way if there are no custom latin rules we won't trigger
                 * a warning when we fall back to the internal latin rules */
                is_latin =
    ...
    if (status == RFPS_NOT_FOUND) {
        if (is_latin) {
            gregorio_messagef(
                ...
                VERBOSITY_INFO,
                0,
                _("Using default Latin vowel rules")
            );
        } else {
            gregorio_messagef(
                "read_rules",
                VERBOSITY_WARNING,
                0,
                _("Selecting Latin instead of %s"),
                requested_language
            );
        }
    }
