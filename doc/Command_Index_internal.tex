...
\macroname{\textbackslash gre@processvisibility}{\#1\#2\#3\#4\#5}{gregoriotex-common.tex}
Helper macro to process visibility values with phantom support.  Accepts \texttt{visible}, \texttt{invisible}, \texttt{phantom}, and \texttt{hphantom} as input and executes the appropriate command while setting the phantom wrapper counter.

\begin{argtable}
  \#1 & \texttt{visible} & Element is shown normally.\\
  & \texttt{invisible} & Element is hidden and takes no space.\\
  & \texttt{phantom} & Element is hidden but preserves both horizontal and vertical space.\\
  & \texttt{hphantom} & Element is hidden but preserves only horizontal space.\\
  \#2 & command & Command to execute when the element is visible (\eg \verb=\gre@showlyricstrue=).\\
  \#3 & command & Command to execute when the element is invisible (\eg \verb=\gre@showlyricsfalse=).\\
  \#4 & count & Phantom wrapper count variable to set (\eg \verb=\gre@lyrics@phantomwrapper=).\\
  \#5 & string & Macro name for error messages (\eg \verb=\protect\gresetlyrics=).\\
\end{argtable}

\macroname{\textbackslash gre@processvisibilitynophantom}{\#1\#2\#3\#4}{gregoriotex-common.tex}
Simplified version of \verb=\gre@processvisibility= for commands that do not support phantom options.  Only accepts \texttt{visible} and \texttt{invisible}.

\begin{argtable}
  \#1 & \texttt{visible} & Element is shown normally.\\
  & \texttt{invisible} & Element is hidden.\\
  \#2 & command & Command to execute when visible.\\
  \#3 & command & Command to execute when invisible.\\
  \#4 & string & Macro name for error messages.\\
\end{argtable}

\macroname{\textbackslash gre@processvisibilitywithphantom}{\#1\#2\#3\#4\#5}{gregoriotex-common.tex}
Simplified version of \verb=\gre@processvisibility= that accepts \texttt{visible}, \texttt{invisible}, and \texttt{phantom} (but not \texttt{hphantom}).  Unlike the full version, both \texttt{visible} and \texttt{phantom} execute the same visibility command, differing only in the phantom wrapper counter value.

\begin{argtable}
  \#1 & \texttt{visible} & Element is shown normally.\\
  & \texttt{invisible} & Element is hidden and takes no space.\\
  & \texttt{phantom} & Element is hidden but preserves horizontal space.\\
  \#2 & command & Command to execute when visible or phantom.\\
  \#3 & command & Command to execute when invisible.\\
  \#4 & count & Phantom wrapper count variable to set.\\
  \#5 & string & Macro name for error messages.\\
\end{argtable}

\macroname{\textbackslash gre@applyphantomwrapper}{\#1\#2}{gregoriotex-common.tex}
Applies a \TeX\ phantom wrapper around content based on a counter value.  Used to make rendered content invisible while preserving its spacing.

\begin{argtable}
  \#1 & \texttt{0} & No wrapper; content is typeset normally.\\
  & \texttt{1} & Wraps content in \verb=\phantom=, preserving both horizontal and vertical space.\\
  & \texttt{2} & Wraps content in \verb=\hphantom=, preserving only horizontal space.\\
  \#2 & content & The material to be (optionally) wrapped.\\
\end{argtable}
...
\macroname{\textbackslash gre@typesettextbelowlines}{\#1}{gregoriotex-main.tex}
Macro for typesetting the nabc neumes below the lines.

\begin{argtable}
  \#1 & string & nabc neumes to be typeset\\
\end{argtable}
...
\macroname{\textbackslash gre@staffdimensions@unzero}{}{gregoriotex-main.tex}
Restores the staff dimensions (line height, inter-line space, staff height, and constant glyph raise) from their saved values if they were previously zeroed.  Only takes effect when \verb=\ifgre@staffdimensions@zeroed= is true; otherwise it is a no-op.

\macroname{\textbackslash gre@staffdimensions@dozero}{}{gregoriotex-main.tex}
Zeroes the staff dimensions (line height, inter-line space, staff height, and constant glyph raise) so that all vertical positioning collapses.  The pre-zeroed values are preserved in \verb=\gre@dimen@stafflineheight@saved=, \verb=\gre@dimen@interstafflinespace@saved=, and \verb=\gre@dimen@staffheight@saved= for later restoration.  Only takes effect when \verb=\ifgre@staffdimensions@zeroed= is false; otherwise it is a no-op.
...
\macroname{\textbackslash gre@applynotesphantom}{\#1\#2}{gregoriotex-syllable.tex}
Notes-specific phantom wrapper that keeps NABC visible while making notes invisible.  It sets \verb=\ifgre@notes@invisrender= to true so that glyph rendering code suppresses note output, then typesets the content into a box and adjusts its dimensions according to the phantom mode.

\begin{argtable}
  \#1 & \texttt{0} & No wrapper (pass-through).\\
  & \texttt{1} & Phantom mode: content is invisible but preserves full dimensions.\\
  & \texttt{2} & Horizontal phantom mode: vertical dimensions (height and depth) are zeroed.\\
...
\macroname{\textbackslash gre@nabconlyglyph}{\#1\#2\#3\#4\#5\#6\#7}{gregoriotex-syllable.tex}
Zero-width glyph handler used by \verb=\gre@notes@rendernabc@tight=.  Triggers NABC rendering and computes the alignment center for the first glyph, but does not produce any note glyph output or advance the cursor.

\begin{argtable}
  \#1--\#7 & various & Same arguments as \verb=\GreGlyph=; all are consumed but only \#4 is used (for alignment center calculation on the first glyph).\\
\end{argtable}
...
\macroname{\textbackslash gre@noopglyph}{\#1\#2\#3\#4\#5\#6\#7}{gregoriotex-syllable.tex}
No-op glyph handler that consumes the same arguments as \verb=\GreGlyph= but produces no visible output.  Used as a replacement for \verb=\GreGlyph= inside \verb=\gre@notes@rendernabc= to suppress note rendering while still allowing NABC callbacks to fire.

\macroname{\textbackslash gre@noopglyphheights}{\#1\#2}{gregoriotex-syllable.tex}
No-op glyph heights handler that consumes the same arguments as \verb=\GreGlyphHeights= but produces no output.  Used as a replacement for \verb=\GreGlyphHeights= when suppressing note rendering.

\macroname{\textbackslash gre@suppress@bars}{}{gregoriotex-syllable.tex}
Redefines all bar glyph macros (\verb=\GreDivisioMinimis=, \verb=\GreDivisioMinor=, etc.) to no-ops so that bars produce no output.  Used inside \verb=\gre@notes@rendernabc= and \verb=\gre@notes@rendernabc@tight=.

\macroname{\textbackslash gre@notes@rendernabc}{\#1}{gregoriotex-syllable.tex}
Renders the notes content with note glyphs replaced by no-ops and bars suppressed, so that only NABC callbacks execute.  Unlike \verb=\gre@notes@rendernabc@tight=, this version preserves inter-element and inter-glyph spacing.
...
\macroname{\textbackslash gre@nabcvoicechar}{[\optional{\#1}]\{\#2\}}{gregoriotex-nabc.tex}
Prints the nabc glyphs for the given nabc string.

\begin{argtable}
  \#1 & integer & Optional. The nabc voice: 1 (default) = above staff, 2 = below staff.\\
...
skip-alterations is enabled), called from \verb=\gre@newglyphcommon=.  When set
by the deferred variant, guarded by \verb=\ifgre@nabc@in@alteration=: execution
is postponed during free-standing alterations and fires at the next real glyph.
...
skip-alterations is enabled), called from \verb=\gre@newglyphcommon=.  Mirrors
...
\begin{argtable}
  \#1 & \texttt{true} & Enable skip-alterations for all voices\\
  & \texttt{false} & Disable skip-alterations for all voices (default)\\
\end{argtable}

\macroname{\textbackslash gre@setnabcskipalterations@voice}{[\#1]\{\#2\}}{gregoriotex-nabc.tex}
Sets the skip-alterations flag for a specific NABC voice.  Internal
implementation of \verb=\gresetnabcskipalterations= when called with the
optional voice argument.

\begin{argtable}
  \#1 & integer & The nabc voice number: 1 = above staff, 2 = below staff\\
  \#2 & \texttt{true} & Enable skip-alterations for the given voice\\
  & \texttt{false} & Disable skip-alterations for the given voice\\
\end{argtable}
...
\macroname{\textbackslash gre@setnabcvoicefont}{[\optional{\#1}]\{\#2\#3\}}{gregoriotex-nabc.tex}
Prints the nabc glyphs for the given nabc string.

\begin{argtable}
  \#1 & integer & Optional. The nabc voice: 1 (default) = above staff, 2 = below staff.\\
  \#2 & string & the name of the font, either \texttt{gregall}, \texttt{grelaon}, or \texttt{gresgmodern}\\
  \#3 & integer & point size at which the font should be loaded\\
\end{argtable}
...
\macroname{\textbackslash ifgre@nabc@voice@i@skipalterations}{}{gregoriotex-nabc.tex}
When true, NABC neumes for voice~1 are anchored to the first real square note,
skipping any leading free-standing alteration signs.  Controlled by
\verb=\gresetnabcskipalterations=.

\macroname{\textbackslash ifgre@nabc@voice@ii@skipalterations}{}{gregoriotex-nabc.tex}
When true, NABC neumes for voice~2 are anchored to the first real square note,
skipping any leading free-standing alteration signs.  Mirrors
\verb=\ifgre@nabc@voice@i@skipalterations= for voice~2.
...
they are being triggered by a free-standing alteration sign rather than by a real
...
\macroname{\textbackslash ifgre@nabc@voice@i@altskipped}{}{gregoriotex-nabc.tex}
Per-voice flag set to true by \verb=\GreSetNabcAboveLinesDeferred= pre-phase
when it detects an alteration (via \verb=\ifgre@nabc@in@alteration=).  When the
pre-phase later fires on a real glyph, this flag causes
\verb=\ifgre@nabc@suppress@leftoverflow= to be set before calling
\verb=\gre@nabc@prepare=, suppressing the left-overflow kern.  Cleared after use.

\macroname{\textbackslash ifgre@nabc@voice@ii@altskipped}{}{gregoriotex-nabc.tex}
Per-voice flag for voice~2.  Mirrors \verb=\ifgre@nabc@voice@i@altskipped= but
used by \verb=\GreSetNabcBelowLinesDeferred=.

\macroname{\textbackslash ifgre@nabc@suppress@leftoverflow}{}{gregoriotex-nabc.tex}
Shared flag checked by \verb=\gre@nabc@prepare=.  When true, the left-overflow
kern is suppressed.  Set and cleared by the deferred pre-phase around each call
to \verb=\gre@nabc@prepare= (separately for each voice), so that the NABC neume
does not push a preceding accidental unnecessarily to the left.
...
\macroname{\textbackslash gre@lyrics@phantomwrapper}{}{gregoriotex-syllable.tex}
Count controlling the phantom wrapper mode for lyrics.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=, \texttt{2}~=~\verb=\hphantom=.  Set by \verb=\gresetlyrics=.

\macroname{\textbackslash gre@notes@phantomwrapper}{}{gregoriotex-syllable.tex}
Count controlling the phantom wrapper mode for notes.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=, \texttt{2}~=~\verb=\hphantom=.  Set by \verb=\gresetnotes=.

\macroname{\textbackslash gre@abovelinestext@phantomwrapper}{}{gregoriotex-main.tex}
Count controlling the phantom wrapper mode for above-lines text.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=.  Set by \verb=\gresetabovelinestext=.

\macroname{\textbackslash gre@lines@phantomwrapper}{}{gregoriotex-main.tex}
Count controlling the phantom wrapper mode for staff lines.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=, \texttt{2}~=~\verb=\hphantom=.  Set by \verb=\gresetlines=.

\macroname{\textbackslash gre@translation@phantomwrapper}{}{gregoriotex-main.tex}
Count controlling the phantom wrapper mode for the translation text.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=.  Set by \verb=\gresettranslation=.

\macroname{\textbackslash gre@nabcvoice@i@phantomwrapper}{}{gregoriotex-nabc.tex}
Count controlling the phantom wrapper mode for the first NABC voice.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=.  Set by \verb=\gresetnabc=.

\macroname{\textbackslash gre@nabcvoice@ii@phantomwrapper}{}{gregoriotex-nabc.tex}
Count controlling the phantom wrapper mode for the second NABC voice.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=.  Set by \verb=\gresetnabc=.

\macroname{\textbackslash gre@clef@phantomwrapper}{}{gregoriotex-signs.tex}
Count controlling the phantom wrapper mode for the clef.  Values: \texttt{0}~=~no wrapper, \texttt{1}~=~\verb=\phantom=, \texttt{2}~=~\verb=\hphantom=.  Set by \verb=\gresetclef=.
...
\macroname{\textbackslash gre@dimen@stafflineheight@saved}{}{gregoriotex-main.tex}
Saved (pre-zeroed) value of the staff line height.  This dimension always holds the true computed value of \verb=\gre@dimen@stafflineheight= regardless of whether staff dimensions have been zeroed.  Computed as \texttt{stafflinethicknessbase $\times$ stafflinefactor}.

\macroname{\textbackslash gre@dimen@interstafflinespace}{}{gregoriotex-spaces.tex}
The space between the lines.

\macroname{\textbackslash gre@dimen@interstafflinespace@saved}{}{gregoriotex-main.tex}
Saved (pre-zeroed) value of the inter-staff-line space.  This dimension always holds the true computed value of \verb=\gre@dimen@interstafflinespace= regardless of whether staff dimensions have been zeroed.

\macroname{\textbackslash gre@dimen@staffheight@saved}{}{gregoriotex-main.tex}
