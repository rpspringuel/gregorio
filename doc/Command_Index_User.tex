...
\begin{argtable}
  \#1 & \texttt{enabled} & The staff lines should be printed (default)\\  
  & \texttt{disabled} & The staff lines should not be printed\\  
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\  
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\  
  & \texttt{phantom} & The staff lines are invisible but space is preserved (uses \textbackslash phantom)\\  
  & \texttt{hphantom} & The staff lines are invisible with zero height (uses \textbackslash hphantom)\\  
  & \texttt{vphantom} & The staff lines are invisible with zero width (uses \textbackslash vphantom)
\end{argtable}

\textbf{Note:} The \texttt{visible} and \texttt{invisible} options are deprecated, kept for backward compatibility, and may be removed in the future. Use \texttt{enabled} and \texttt{disabled} instead.
...
\begin{argtable}
  \#1 & \texttt{enabled} & The staff lines behind a punctum cavum should be printed (Plain \TeX\ default)\\
  & \texttt{disabled} & The staff lines behind a punctum cavum should not be printed (\LaTeX\ default)\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & The lines are invisible but space is preserved\\
  & \texttt{hphantom} & The lines are invisible with zero height\\
  & \texttt{vphantom} & The lines are invisible with zero width\\
\end{argtable}
...
\begin{argtable}
  \#1 & \texttt{enabled} & The staff lines behind an alteration should be printed (Plain \TeX\ default)\\
  & \texttt{disabled} & The staff lines behind an alteration should not be printed (\LaTeX\ default)\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & The lines are invisible but space is preserved\\
  & \texttt{hphantom} & The lines are invisible with zero height\\
...
\begin{argtable}
  \#1 & \texttt{enabled} & The staff lines behind a dotted bar should be printed\\
  & \texttt{disabled} & The staff lines behind a dotted bar should not be printed (default)\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & The lines are invisible but space is preserved\\
  & \texttt{hphantom} & The lines are invisible with zero height\\
...
\begin{tabular}{lp{7cm}r}
  ...
  \stylename{abovelinesnabc} & ancient notation (above staff) & none\\
  \stylename{belowlinesnabc} & ancient notation (below staff) & none\\
...
\begin{argtable}
  \#1 & \texttt{enabled} & Lyrics are visible (default).\\
  & \texttt{disabled} & Lyrics are not visible (no space occupied).\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & Lyrics are invisible but space is preserved.\\
  & \texttt{hphantom} & Lyrics are invisible with zero height.\\
  & \texttt{vphantom} & Lyrics are invisible with zero width.\\
\end{argtable}
...
\begin{argtable}
  \#1 & \texttt{enabled} & Translations are visible (default).\\
  & \texttt{disabled} & Translations are not visible (no space occupied).\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & Translations are invisible but space is preserved.\\
  & \texttt{hphantom} & Translations are invisible with zero height.\\
  & \texttt{vphantom} & Translations are invisible with zero width.\\
\end{argtable}
...
\begin{argtable}
  \#1 & \texttt{enabled} & Above lines text are visible (default).\\
  & \texttt{disabled} & Above lines text are not visible (no space occupied).\\
  & \texttt{visible} & same as \texttt{enabled} (see note below).\\
  & \texttt{invisible} & same as \texttt{disabled} (see note below).\\
  & \texttt{phantom} & Above lines text are invisible but space is preserved.\\
  & \texttt{hphantom} & Above lines text are invisible with zero height.\\
  & \texttt{vphantom} & Above lines text are invisible with zero width.\\
\end{argtable}
...
\begin{argtable}
  \#1 & \texttt{enabled} & The custos alteration should be printed (default)\\
      & \texttt{disabled} & The custos alteration should not be printed\\
      & \texttt{visible} & The custos alteration should be printed (deprecated, use \texttt{enabled})\\
      & \texttt{invisible} & The custos alteration should not be printed (deprecated, use \texttt{disabled})\\
      & \texttt{phantom} & The custos alteration is invisible but space is preserved\\
      & \texttt{hphantom} & The custos alteration is invisible with zero height\\
      & \texttt{vphantom} & The custos alteration is invisible with zero width\\
\end{argtable}

\textbf{Note:} The \texttt{visible} and \texttt{invisible} options are deprecated and will be removed in version 7.0.0. Use \texttt{enabled} and \texttt{disabled} instead.
...
\begin{argtable}
  \#1 & \texttt{enabled} & Notes are visible (default).\\
  & \texttt{disabled} & Notes are not visible (no space occupied).\\
  & \texttt{visible} & Notes are visible (deprecated, use \texttt{enabled}).\\
  & \texttt{invisible} & Notes are not visible (deprecated, use \texttt{disabled}).\\
  & \texttt{phantom} & Notes are invisible but space is preserved.\\
  & \texttt{hphantom} & Notes are invisible with zero height. Useful for NABC with lyrics only.\\
  & \texttt{vphantom} & Notes are invisible with zero width.\\
\end{argtable}

\textbf{Note:} The \texttt{visible} and \texttt{invisible} options are deprecated and will be removed in version 7.0.0. Use \texttt{enabled} and \texttt{disabled} instead.

\textbf{Nota Bene:} If the notes are set to be invisible, then bar lines, rythmic signs, and the like will also be invisible. However, the staff lines and clefs will still show up (since their visibility is controlled by other settings). NABC neumes are also unaffected and remain controlled independently by \verb=\gresetnabc=.

\macroname{\textbackslash gresetnabc}{\{\#1\}\{\#2\}}{gregoriotex-nabc.tex}
Sets the visibility of an nabc voice.

\begin{argtable}
  \#1 & integer & The nabc voice number: 1 = above staff, 2 = below staff.\\
  \#2 & \texttt{enabled} & Notes are visible (default).\\
  & \texttt{disabled} & Notes are not visible (no space occupied).\\
  & \texttt{visible} & Notes are visible (deprecated, use \texttt{enabled}).\\
  & \texttt{invisible} & Notes are not visible (deprecated, use \texttt{disabled}).\\
  & \texttt{phantom} & Notes are invisible but space is preserved.\\
  & \texttt{hphantom} & Notes are invisible with zero height.\\
  & \texttt{vphantom} & Notes are invisible with zero width.\\
\end{argtable}

\textbf{Note:} The \texttt{visible} and \texttt{invisible} options are deprecated and will be removed in version 7.0.0. Use \texttt{enabled} and \texttt{disabled} instead.

\textbf{Important:} NABC visibility is controlled independently from notes visibility (\verb=\gresetnotes=). This allows you to hide the main notes (e.g., using \verb=\gresetnotes{hphantom}=) while keeping NABC neumes visible, which is useful for producing NABC-only output with lyrics.
...
\begin{argtable}
  \#1 & \texttt{enabled} & Clefs will be printed (default)\\
  & \texttt{disabled} & Clefs will not be printed\\
  & \texttt{visible} & Clefs will be printed (deprecated, use \texttt{enabled})\\
  & \texttt{invisible} & Clefs will not be printed (deprecated, use \texttt{disabled})\\
  & \texttt{phantom} & Clefs are invisible but space is preserved\\
  & \texttt{hphantom} & Clefs are invisible with zero height\\
  & \texttt{vphantom} & Clefs are invisible with zero width\\
\end{argtable}
...
\macroname{\textbackslash gresetnabcfont}{[\#1]\{\#2\}\{\#3\}}{gregoriotex-nabc.tex}
Macro to set the font to be used for the ancient notation.

\begin{argtable}
  \#1 & integer & the nabc voice: 1 (default) = above staff, 2 = below staff.\\
...
\def\getdims{%
  ...
  \edef\abovelinesnabcheight{\gre@space@dimen@abovelinesnabcheight}%
  \edef\abovelinesnabcraise{\gre@space@dimen@abovelinesnabcraise}%
  ...
  \edef\belowlinesnabcheight{\gre@space@dimen@belowlinesnabcheight}%
  ...
  \scalebox{2}{%
    ...
    \begin{mplibcode}
      beginfig(0);
        ...
        y3 := y2+\belowlinesnabcheight;
        y4 := y3+\staffheight;
        y5 := y4+\abovelinesnabcraise;
        y6 := y5+\abovelinesnabcheight;
        y7 := y6+\abovelinestextraise;
        y8 := y7+\abovelinestextheight;
        y9 := y8+\spaceabovelines;
        x := 0.1cm;
        draw (-x, 0) -- (x, 0);
        draw (-x,y0) -- (x,y0);
        draw (-x,y1) -- (x,y1);
        ...
        drawdblarrow ( 0,y2) -- (0,y3); label.lft(btex \lab{belowlinesnabcheight} etex, (0, (y2+y3)/2));
        %drawdblarrow ( 0,y3) -- (0,y4); label.lft(btex \lab{staffheight} etex, (0, (y3+y4)/2));
        drawdblarrow ( 0,y4) -- (0,y5); label.lft(btex \lab{abovelinesnabcraise} etex, (0, (y4+y5)/2));
        drawdblarrow ( 0,y5) -- (0,y6); label.lft(btex \lab{abovelinesnabcheight} etex, (0, (y5+y6)/2));
        drawdblarrow ( 0,y6) -- (0,y7); label.lft(btex \lab{abovelinestextraise} etex, (0, (y6+y7)/2));
        drawdblarrow ( 0,y7) -- (0,y8); label.lft(btex \lab{abovelinestextheight} etex, (0, (y7+y8)/2));
        drawdblarrow ( 0,y8) -- (0,y9); label.lft(btex \lab{spaceabovelines} etex, (0, (y8+y9)/2));
      endfig;
    \end{mplibcode}%
    \hspace{0.2cm}%
    \lines{\nabcsnippet{2}{<alt>alt</alt>text[trans](g|vi|vi)}}%
  }
\end{center}
...
The distance from the baseline of the above-lines text down to the baseline of the above-lines NABC neumes (if any) or the top staff line (otherwise). This space appears only if there actually is above-lines text.
\end{gdimension}

\begin{gdimension}{abovelinesnabcheight}
The height of the above-lines NABC neumes. This space appears only if there actually is above-lines NABC, and it must be larger than the above-line NABC's natural height for it to have an effect.
\end{gdimension}

\begin{gdimension}{abovelinesnabcraise}
The distance from the baseline of the above-lines NABC neumes down to the top staff line. This space appears only if there actually is above-lines NABC.
\end{gdimension}

\begin{gdimension}{spacelinestext}
The space between the staff lines and the baseline of the text. This space appears whether or not there is actually text.
\end{gdimension}

\begin{gdimension}{belowlinesnabcheight}
The space between the baseline of the text and the baseline of the below-lines NABC neumes (if any). This space appears only if there actually is below-lines NABC.
\end{gdimension}

\begin{gdimension}{translationheight}
The space between the baseline of the translation and the baseline of the below-lines NABC neumes (if any) or text (otherwise). This space appears only if there actually is a translation.
\end{gdimension}

\begin{gdimension}{spacebeneathtext}
The space beneath the text (and beneath the NABC neumes and/or translation, if any). It must be larger than the natural depth of the text (or NABC neumes or translation) for it to have an effect.
\end{gdimension}
...
\ttfamily
\begin{tabular}{ccccc}
  \lines{\nabcsnippet{1}{m|vi[0 steps](m|vi)}} &
  \lines{\nabcsnippet{1}{m_1|vi[0 steps](m_1|vi)}} &
  \lines{\nabcsnippet{1}{m<v>\textquotesingle</v>1|vi[1 step](m'1|vi)}} &
  \lines{\nabcsnippet{1}{m_1<v>\textquotesingle</v>1|vi[2 steps](m_1'1|vi)}}
\end{tabular}
\end{center}
\end{quote}
