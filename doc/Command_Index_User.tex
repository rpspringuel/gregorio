...
\begin{argtable}
  ...
  & \texttt{phantom} & The staff lines are invisible but space is preserved\\
  & \texttt{hphantom} & The staff lines are invisible with zero height\\
  & \texttt{vphantom} & The staff lines are invisible with zero width but height is preserved
\end{argtable}

\textbf{Note:} With \texttt{invisible}, NABC voice 1 (above staff) and above-lines text are moved down closer to the lyrics line. With \texttt{phantom}, the staff lines are not rendered but NABC voice 1 and above-lines text maintain their original vertical positions as if the staff lines were present. This is useful for producing output with NABC neumes or above-lines text at the correct height without visible staff lines.
...
\begin{argtable}
  ...
  & \texttt{invisible} & Lyrics are not visible (no space occupied).\\
...
\begin{argtable}
  ...
  & \texttt{invisible} & Translations are not visible (no space occupied).\\
...
\begin{argtable}
  ...
  & \texttt{invisible} & Above lines text are not visible (no space occupied).\\
...
\macroname{\textbackslash grebolshift}{}{gregoriotex-spaces.tex}
Macro to return the current beginning-of-line shift (bolshift) as a dimension value. This can be reused when setting distances, for example with \verb=\grechangedim=.
...
\begin{argtable}
  ...
  & \texttt{invisible} & Notes are not visible (no space occupied).\\
...
\begin{argtable}
  ...
  \#2 & \texttt{visible} & NABC glyphs are visible (default).\\
  & \texttt{invisible} & NABC glyphs are not visible (no space occupied).\\
  & \texttt{phantom} & NABC glyphs are invisible but space is preserved.
\end{argtable}
...
\textbf{Nota Bene:} For "NABC + lyrics only" layouts, you can hide the clef while keeping line alignment by combining \verb=\gresetclef{invisible}= with \verb=\grechangedim{noclefspace}{-\grebolshift}{scalable}=.
