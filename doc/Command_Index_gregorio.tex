...
Deferred variant of \verb=\GreSetNabcAboveLines= used when skip-alterations is
enabled for voice~1 (\verb=\gresetnabcskipalterations[1]{true}=).  The
pre-phase is guarded by \verb=\ifgre@nabc@in@alteration= so it silently
postpones execution during free-standing alteration signs and fires only at
the next real \verb=\GreGlyph= call, anchoring the neume to the first square
note.  When an alteration is skipped, the per-voice
\verb=\ifgre@nabc@voice@i@altskipped= flag is set, causing the left-overflow
kern in \verb=\gre@nabc@prepare= to be suppressed (via
\verb=\ifgre@nabc@suppress@leftoverflow=), since the accidental already
provides visual space before the note.  The emit-phase is likewise guarded by
\verb=\ifgre@nabc@voice@i@ready= to prevent placing an empty box before the
pre-phase has run.
...
Deferred variant of \verb=\GreSetNabcBelowLines= used when skip-alterations is
enabled for voice~2 (\verb=\gresetnabcskipalterations[2]{true}=).  Mirrors
\verb=\GreSetNabcAboveLinesDeferred= but targets voice~2 (below staff), using
\verb=\ifgre@nabc@voice@ii@altskipped=, \verb=\ifgre@nabc@voice@ii@ready=
and the \verb=\gre@nabc@*@voice@ii= macros.
...
\verb=\gresetnabcskipalterations= is enabled for that voice).
